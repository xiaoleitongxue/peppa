...
void YoloObjectDetector::yolo() {
  ...
  while (!demoDone_) {
    // buffIndex points second element in buff array
    ...
    if (!demoPrefix_) {
      ...
      demoTime_ = what_time_is_it_now();
      // check wether a image can be view
      if (viewImage_) {
        displayInThread(0);
      } else {
